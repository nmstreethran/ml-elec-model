...
for state in states:
    ...
    for stn, sd, ed in stations:
        ...
        stn = stn.zfill(5)
        dest = path+'/'+stn # file download directory
        if start < yr or end < yr: # historical data (not current year)
            ...
            url = (repourl+'historical/stundenwerte_FF_'+stn+'_'+sd_str+'_'
                +ed_str+'_hist.zip') # zip file url
        else: # recent data (current year) zip file url
